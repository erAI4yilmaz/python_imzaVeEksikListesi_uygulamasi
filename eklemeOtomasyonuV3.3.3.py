        ...
        try:
            ...
            for d_index, excel_yolu in enumerate(self.secilen_excel_yollari):
                ...
                for s_index, sayfa_adi in enumerate(sayfalar):
                    ...
                    isim = sayfa_adi.lower().strip()
                    isim = isim.replace("i\u0307", "i")  # Türkçe İ.lower() düzeltmesi
                    
                    gerekli_sayfa = any(k in isim for k in [
"kompresör","aed","araç taşıyıcı","caraskal","cephe asansörü","iskele",
"çektirme","ytt","forklift","yoyo","araç kaldırma lifti","istif mak.","istif makinesi",
...
"yürüyen merdiven","sabit iniş mah",

"basınçlı hava tankı","otoklav","basınçlı kap","bk emniyet cihazı","yıkama mak.",
...
"ütü kaz.","yemek pişirme kazanı","emniyet kemeri",

"bükme","chiller","genel","cnc kesme","diş çekme","eğlence","enjeksiyon",
"erezyon","ısı eşanjörü","freze","demir kesme","kaynak","makas",
"matkap","planya","pres","taşlama","torna","zımpara","kamyon","arazöz","dozer"
])
                    ...
                    #===========KALDIRMA EKİPMANLARI======================
                        ...
                        if "kompresör" in isim:
                        ...
                        elif "istif mak." in isim or "istif makinesi" in isim:
                           data = [
                           ...
                           satir += 1 
                                ...
                                self._excel_veriyi_yaz_ram(hedef_ws, satir, data, sheet_data, range(40, 60))
                                satir += 1
                    ...
                    #yükseltilebilir iş platformu vardı -------------
                        ...
                        ###=============BASINÇLI KAPLAR===========================================
                        ...
                        elif "yıkama mak." in isim or "yıkama makinası" in isim:
                           data = [
                           ...
                           data = [
                               ...
                               self.hucre_oku(sheet_data, "V24"),     # C → İşletmedeki yeri
                               self.hucre_oku(sheet_data, "BO21"),     # D → Seri no
                               self.hucre_oku(sheet_data, "V21"),     # E → Markası
                               self.hucre_oku(sheet_data, "DB21"),     # F → Kapasitesi
                                   ]
                           self._excel_veriyi_yaz_ram(hedef_ws, satir, data, sheet_data, range(54, 58))
                           satir += 1
                        ...
                        elif "otoklav" in isim:
                           data = [
                               "Otoklav",                       # B → sabit metin
                               self.hucre_oku(sheet_data, "V22"),     # C → İşletmedeki yeri
                               self.hucre_oku(sheet_data, "BO18"),     # D → Seri no
                               self.hucre_oku(sheet_data, "V18"),     # E → Markası
                               self.hucre_oku(sheet_data, "DB18"),     # F → Kapasitesi
                                   ]
                           self._excel_veriyi_yaz_ram(hedef_ws, satir, data, sheet_data, range(53, 58))
                           satir += 1
                        ...
                        #=============TEZGAH===========================
                           ...
                           data = [
                               self.hucre_oku(sheet_data, "BO15"),                       # B → sabit metin
                               self.hucre_oku(sheet_data, "BO16"),     # C → İşletmedeki yeri
                               self.hucre_oku(sheet_data, "R16"),     # D → Seri no
                               self.hucre_oku(sheet_data, "R13"),     # E → Markası
                               self.hucre_oku(sheet_data, "BO13"),     # F → Kapasitesi
    ...
    def _excel_veriyi_yaz_ram(self, hedef_ws, satir, data, sheet_data, d_araligi):
        ...
        # DÜZELTME: Sadece "1) ...", "2) ..." formatındaki gerçek kusurları al
        notlar = []
        for r in d_araligi:
            val = self.hucre_oku(sheet_data, f"A{r}")
            if val and re.match(r'^\d+\)\s+\S', str(val).strip()):
                notlar.append(str(val).strip())
